...
const heroImage = "https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?auto=format&fit=crop&w=1500&q=92";
...
export default function Home({ coffees, onLogin }: HomeProps) {
  ...
  const popular = useMemo(() => {
    const ordered = popularOrder.map((name) => available.find((coffee) => coffee.name.toLowerCase() === name.toLowerCase())).filter(Boolean) as Coffee[];
  ...
  return <main className="reference-home">
    <section className="ref-hero" id="home">
      <div className="ref-hero-content">
        <div className="ref-rating"><span className="google-g">G</span><strong>Google</strong><span className="ref-stars">★★★★★</span><b>(4.9)</b></div>
        ...
        <div className="ref-hero-actions"><button className="ref-primary" onClick={() => navigate("/menu")}>Explore menu <span>→</span></button><button className="ref-story-play" onClick={() => navigate("/story")} aria-label="Watch our story">▶</button><button className="ref-story-link" onClick={() => navigate("/story")}>Watch our story</button></div>
      </div>
      <div className="ref-hero-media" aria-hidden="true"><div className="ref-hero-glow" /><img src={heroImage} alt="" /></div>
    </section>
    ...
    <section className="ref-popular" id="popular"><div className="ref-section-title"><div><small>CURATED FOR YOU</small><h2>Our Popular Picks <em>⌁</em></h2></div><button onClick={() => navigate("/menu")}>View all menu <span>→</span></button></div>
      <div className="ref-popular-grid">{popular.map((coffee) => <article className="ref-product-card" key={coffee.id}><div className="ref-product-image"><img src={getImage(coffee.name)} alt={coffee.name} onError={(event) => { event.currentTarget.src = fallback; }} /><button className="ref-heart" aria-label={`Favourite ${coffee.name}`}>♡</button></div><div className="ref-product-body"><h3>{coffee.name}</h3><strong>₹{coffee.price}</strong><button className="ref-add-round" onClick={() => addToCart(coffee)} aria-label={`Add ${coffee.name} to cart`}>+</button></div></article>)}</div>
    </section>

    <section className="ref-promo" id="contact"><div className="ref-promo-copy"><strong>10% <small>OFF</small></strong><span><b>Get 10% off on your first order!</b><small>Join our coffee community and enjoy exclusive offers.</small></span></div><button onClick={onLogin}>Join Now <span>→</span></button></section>
  </main>;
}
