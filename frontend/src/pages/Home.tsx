...
export default function Home({ coffees, onLogin }: HomeProps) {
  ...
  const featured = useMemo(() => {
    const selected = preferred.map((name) => available.find((c) => c.name.toLowerCase() === name.toLowerCase())).filter(Boolean) as Coffee[];
    return selected.length ? selected : available.slice(0, 5);
  }, [available]);
    ...
    <main className="professional-home">
      <section className="home-hero">
        ...
        <div className="home-hero-copy">
          ...
          <div className="hero-buttons">
            <button className="home-primary" onClick={() => navigate("/menu")}>Explore our menu <b>↗</b></button>
            <button className="home-link" onClick={() => navigate("/story")}>Our story <b>→</b></button>
          </div>
          <div className="hero-meta"><span><b>4.9</b> ★★★★★</span><span>•</span><span>LOVED BY COFFEE PEOPLE</span></div>
        </div>
        <div className="hero-side-label">BREWED<br />WITH INTENTION</div>
      </section>

      <section className="trust-strip">
        <div><strong>100%</strong><span>Premium beans</span></div>
        <div><strong>24H</strong><span>Fresh roast promise</span></div>
        <div><strong>5★</strong><span>Every cup matters</span></div>
        <div><strong>∞</strong><span>Good conversations</span></div>
      </section>

      <section className="home-story">
        <div className="section-number">01 <span>OUR PHILOSOPHY</span></div>
        <div className="story-copy">
          <p className="section-kicker">THE COFFEEHUB WAY</p>
          <h2>Coffee is a<br /><i>daily ritual.</i></h2>
          <p>We created CoffeeHub for the pause between busy moments. From the beans we choose to the way we serve them, everything is considered so your coffee can simply taste great.</p>
          <button className="outline-button" onClick={() => navigate("/story")}>Discover CoffeeHub <span>→</span></button>
        </div>
      ...
      <section className="featured-section">
        <div className="section-heading-row">
          <div><p className="section-kicker">THE HOUSE MENU</p><h2>Favourites, <i>made fresh.</i></h2></div>
          <button className="home-link" onClick={() => navigate("/menu")}>View full menu <b>↗</b></button>
        </div>
        <div className="coffee-grid">
          {featured.map((coffee, index) => (
            <article className={`coffee-tile ${index === 0 ? "featured" : ""}`} key={coffee.id}>
              <button className="coffee-tile-photo" onClick={() => navigate(`/product/${coffee.id}`)} aria-label={`View ${coffee.name}`}>
                <img src={getCoffeeImage(coffee.name)} alt={coffee.name} onError={(event) => { event.currentTarget.src = fallback; }} />
                <span className="tile-number">0{index + 1}</span><span className="tile-view">VIEW ↗</span>
              </button>
              <div className="coffee-tile-body">
                <small>{coffee.category || "SPECIALTY COFFEE"}</small>
                <h3>{coffee.name}</h3>
                <div><strong>₹{coffee.price}</strong><button className="tile-add" onClick={() => addToCart(coffee)} aria-label={`Add ${coffee.name} to cart`}>+</button></div>
              </div>
            </article>
          ))}
        </div>
      ...
      <section className="experience-section">
        <div className="experience-photo"><img src={images.atmosphere} alt="CoffeeHub cafe atmosphere" /><span>YOUR TABLE<br />IS WAITING</span></div>
        <div className="experience-copy">
          <p className="section-kicker">THE EXPERIENCE</p>
          <h2>Stay for the<br /><i>atmosphere.</i></h2>
          <p>Warm light. Fresh coffee. A playlist that never gets in the way of conversation. CoffeeHub is designed to feel like your favourite corner of the city.</p>
          <div className="experience-points"><div><b>01</b><span><strong>Slow mornings</strong><small>Start the day properly.</small></span></div><div><b>02</b><span><strong>Good company</strong><small>Bring someone you like.</small></span></div><div><b>03</b><span><strong>Great coffee</strong><small>Obviously.</small></span></div></div>
          <button className="outline-button" onClick={() => navigate("/contact")}>Find us <span>→</span></button>
        </div>
      </section>

      <section className="craft-section">
        <div className="craft-copy"><p className="section-kicker">WHY OUR COFFEE</p><h2>Details make<br /><i>the difference.</i></h2><p>We keep our process simple and our standards high. Better beans, careful roasting and a barista who takes the extra few seconds to get it right.</p><div className="craft-list"><div><b>01</b><span><strong>Premium sourcing</strong><small>Exceptional beans from trusted origins.</small></span></div><div><b>02</b><span><strong>Small-batch roasting</strong><small>Balanced for sweetness, body and clarity.</small></span></div><div><b>03</b><span><strong>Brewed to order</strong><small>Fresh, precise and never rushed.</small></span></div></div></div>
        <div className="craft-photo"><img src={images.craft} alt="Coffee being brewed" /><span>CRAFTED<br />DAILY</span></div>
      </section>
