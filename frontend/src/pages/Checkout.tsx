...
function Checkout() {
  ...
  const [form, setForm] = useState({ name: customer?.name || "", phone: customer?.phone || "", address: customer?.address || "", city: customer?.city || "", pincode: customer?.pincode || "" });
  ...
  if (!localStorage.getItem("customer_access_token")) return (
  ...
  return (
    <main className="coffee-page min-h-screen py-12 px-5"><div className="coffee-container max-w-6xl mx-auto"><button onClick={() => navigate("/")} className="coffee-muted hover:text-[var(--coffee-brown)] mb-6">← Back to menu</button><div className="grid lg:grid-cols-[1.3fr_0.7fr] gap-8 items-start">
      <section className="coffee-card bg-white p-7 sm:p-9"><p className="text-[10px] uppercase tracking-[0.3em] text-[var(--coffee-brown)]">Checkout</p><h1 className="text-4xl font-bold coffee-heading mt-2">Almost yours.</h1><p className="coffee-muted mt-3 mb-8">{profileLoading ? "Loading your saved delivery details..." : "Your saved delivery details are ready. You can edit them for this order."}</p>
        <div className="grid sm:grid-cols-2 gap-5"><div><label className="coffee-label">Full name</label><input className="coffee-input" value={form.name} onChange={(e) => update("name", e.target.value)} required placeholder="Your name" /></div><div><label className="coffee-label">Phone</label><input className="coffee-input" value={form.phone} onChange={(e) => update("phone", e.target.value.replace(/\D/g, "").slice(0, 10))} required placeholder="9876543210" inputMode="tel" /></div><div className="sm:col-span-2"><label className="coffee-label">Delivery address</label><textarea className="coffee-input min-h-28 resize-none" value={form.address} onChange={(e) => update("address", e.target.value)} required placeholder="House / Flat, street, area" /></div><div><label className="coffee-label">City</label><input className="coffee-input" value={form.city} onChange={(e) => update("city", e.target.value)} required placeholder="Your city" /></div><div><label className="coffee-label">Pincode</label><input className="coffee-input" inputMode="numeric" maxLength={6} value={form.pincode} onChange={(e) => update("pincode", e.target.value.replace(/\D/g, "").slice(0, 6))} required placeholder="520001" /></div></div>
        <div className="mt-8 rounded-2xl bg-[#fbf7f2] border border-black/5 p-5"><p className="text-sm font-semibold coffee-heading">Payment</p><p className="text-sm coffee-muted mt-1">Cash on delivery is available for this checkout.</p></div>{error && <div className="rounded-xl bg-red-50 border border-red-100 text-red-700 px-4 py-3 text-sm mt-5">{error}</div>}<button onClick={placeOrder} disabled={loading || !canSubmit} className="coffee-button w-full justify-center mt-7 disabled:opacity-40 disabled:cursor-not-allowed">{loading ? "Placing order..." : `Place order · ₹${total}`}</button>
      </section>
