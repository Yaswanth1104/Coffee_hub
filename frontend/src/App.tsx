...
function HomePage() {
  const [coffees, setCoffees] = useState<Coffee[]>([]); const [loading, setLoading] = useState(true); const [error, setError] = useState("");
  const [cartItems, setCartItems] = useState<CartItem[]>(() => { try { return JSON.parse(localStorage.getItem("coffeehub_cart") || "[]"); } catch { return []; } });
  const [cartOpen, setCartOpen] = useState(false); const navigate = useNavigate();
  ...
  const increase = (id: number) => setCartItems(items => items.map(i => i.id === id ? { ...i, quantity: i.quantity + 1 } : i));
  const decrease = (id: number) => setCartItems(items => items.flatMap(i => i.id !== id ? [i] : i.quantity > 1 ? [{ ...i, quantity: i.quantity - 1 }] : []));
  const remove = (id: number) => setCartItems(items => items.filter(i => i.id !== id)); const cartCount = cartItems.reduce((s, i) => s + i.quantity, 0);
